      ...
      {techs.map((tech) => {
        ...
        return (
          <div
            key={tech.id}
            className={cn(
              "group relative flex flex-col gap-3 rounded-[var(--radius-lg)] border p-4 transition-all duration-150",
              isSelected
          ...
          >
            ...
            {/* A native <button> cannot contain the "Doc" <a> below (invalid HTML,
                fails WCAG 4.1.2) — the toggle target only wraps the non-link content. */}
            <button type="button" onClick={() => onToggle(tech.id)} aria-pressed={isSelected} className="flex flex-col gap-3 text-left">
